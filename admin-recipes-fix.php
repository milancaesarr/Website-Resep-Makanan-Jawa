        ...
        if (empty($name) || empty($description) || empty($region)) {
            ...
            if ($action === 'edit') {
                ...
                if ($current_stmt) {
                    ...
                    $current_recipe = mysqli_fetch_assoc($current_result);
                ...
                if (in_array($file_ext, $allowed_types)) {
                    ...
                    if ($_FILES['recipe_image']['size'] <= 5 * 1024 * 1024) {
                        if (move_uploaded_file($file_tmp, $upload_path)) {
                            ...
                            if ($action === 'edit' && isset($current_recipe) && !empty($current_recipe['image_url']) &&
                                file_exists($current_recipe['image_url']) &&
                                $current_recipe['image_url'] !== $upload_path) {
                                unlink($current_recipe['image_url']);
                            }
            ...
            if ($action === 'add') {
                // INSERT - berdasarkan screenshot kolom database
                $sql = "INSERT INTO recipes (name, description, region, prep_time, cook_time, difficulty, rating, votes, image_url, author, cooking_method, cuisine, courses, ingredients, ingredients_main, ingredients_seasoning, ingredients_complementary, instructions) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_prepare($conn, $sql);

                if ($stmt) {
                    // 18 parameters sesuai urutan kolom
                    mysqli_stmt_bind_param($stmt, "ssssssdiissssssss",
                        $name, $description, $region, $prep_time, $cook_time, $difficulty,
                        $rating, $votes, $image_url, $author, $cooking_method, $cuisine,
                        $courses, $ingredients, $ingredients_main, $ingredients_seasoning,
                        $ingredients_complementary, $instructions);
                ...
                $id = intval($_POST['id']);
                $sql = "UPDATE recipes SET name=?, description=?, region=?, prep_time=?, cook_time=?, difficulty=?, rating=?, votes=?, image_url=?, author=?, cooking_method=?, cuisine=?, courses=?, ingredients=?, ingredients_main=?, ingredients_seasoning=?, ingredients_complementary=?, instructions=? WHERE id=?";
                $stmt = mysqli_prepare($conn, $sql);

                if ($stmt) {
                    // 19 parameters (18 fields + id)
                    mysqli_stmt_bind_param($stmt, "ssssssdiissssssssi",
                        $name, $description, $region, $prep_time, $cook_time, $difficulty,
                        $rating, $votes, $image_url, $author, $cooking_method, $cuisine,
                        $courses, $ingredients, $ingredients_main, $ingredients_seasoning,
                        $ingredients_complementary, $instructions, $id);
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($recipe['image_url'])): ?>
                                <img src="<?php echo htmlspecialchars($recipe['image_url']); ?>" alt="<?php echo htmlspecialchars($recipe['name']); ?>" class="recipe-image">
                            <?php else: ?>
...
<script>
    ...
    function setupFileUpload(inputId, labelId, previewId) {
        ...
        if (!input || !label || !preview) {
            console.error('File upload elements not found:', inputId, labelId, previewId);
            return;
        }
    ...
    function openModal(action) {
        ...
        if (action === 'add') {
            ...
            if (fileLabel) {
                ...
                if (labelSpan) {
                    labelSpan.textContent = 'Klik untuk upload gambar';
                }
            }
        }
                ...
                if (data.success) {
                    ...
                    if (recipe.image_url) {
                        imagePreview.innerHTML = `
                            ...
                            <img src="${recipe.image_url}" alt="Current Image" style="max-width: 200px; max-height: 150px; border-radius: 8px; border: 2px solid #e0e0e0;">
                        `;
                    } else {
...
// Handle AJAX request for adding/editing recipe
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_recipe_action'])) {
    // Clear any output buffering to prevent HTML output
    while (ob_get_level()) {
        ob_end_clean();
    }
    $action = $_POST['action'] ?? '';
    ...
    if ($action === 'add' || $action === 'edit') {
        ...
        if (empty($name) || empty($description) || empty($region)) {
            ...
            // Handle image upload - SIMPLIFIED VERSION LIKE PRODUCTS
            $image_url = '';
            $current_image = '';
            ...
            // Check if new image is uploaded - USING SAME LOGIC AS PRODUCTS
            if (isset($_FILES['recipe_image']) && $_FILES['recipe_image']['error'] === UPLOAD_ERR_OK) {
                $upload_dir = 'uploads/recipes/';
            ...
            // Database operations - FIXED PARAMETER BINDING
            if ($action === 'add') {
                ...
                if ($stmt) {
                    // FIXED: Correct parameter binding - 17 parameters
                ...
                if ($stmt) {
                    // FIXED: Correct parameter binding - 18 parameters (17 + id)
...
<div class="admin-container">
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="admin-orders.php"><i data-feather="shopping-bag"></i> Pesanan</a></li>
            <li><a href="admin-users.php"><i data-feather="users"></i> Pengguna</a></li>
            <li><a href="admin-recipes.php" class="active"><i data-feather="book-open"></i> Resep</a></li>
            <li><a href="admin-community.php"><i data-feather="message-circle"></i> Komunitas</a></li>
            <li><a href="admin-contacts.php"><i data-feather="mail"></i> Kontak</a></li>
            <li><a href="home-login.php" target="_blank"><i data-feather="external-link"></i> Lihat Website</a></li>
            <li><a href="logout.php" onclick="return confirm('Yakin ingin logout?')"><i data-feather="log-out"></i> Logout</a></li>
